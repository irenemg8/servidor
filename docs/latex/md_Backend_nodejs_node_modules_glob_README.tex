...
\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1310}{}\doxysection{Usage}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1310}
Install with npm
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1311}{}\doxysection{Glob Primer}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1311}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1312}{}\doxysubsection{Dots}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1312}
...
You can make glob treat dots as normal characters by setting {\ttfamily dot\+:true} in the options.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1313}{}\doxysubsection{Basename Matching}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1313}
If you set {\ttfamily match\+Base\+:true} in the options, and the pattern has no slashes in it, then it will seek for any file anywhere in the tree with a matching basename. For example, {\ttfamily $\ast$.js} would match {\ttfamily test/simple/basic.\+js}.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1314}{}\doxysubsection{Empty Sets}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1314}
...
a*s*d*f
\end{DoxyVerb}
 To get the bash-\/style behavior, set the {\ttfamily nonull\+:true} in the options.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1315}{}\doxysubsection{See Also\+:}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1315}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1316}{}\doxysection{glob.\+has\+Magic(pattern, \mbox{[}options\mbox{]})}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1316}
Returns {\ttfamily true} if there are any special characters in the pattern, and {\ttfamily false} otherwise.

Note that the options affect the results. If {\ttfamily noext\+:true} is set in the options object, then {\ttfamily +(a$\vert$b)} will not be considered a magic pattern. If the pattern has a brace expansion, like {\ttfamily a/\{b/c,x/y\}} then that is considered magical, unless {\ttfamily nobrace\+:true} is set in the options.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1317}{}\doxysection{glob(pattern, \mbox{[}options\mbox{]}, cb)}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1317}
...
Perform an asynchronous glob search.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1318}{}\doxysection{glob.\+sync(pattern, \mbox{[}options\mbox{]})}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1318}
...
Perform a synchronous glob search.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1319}{}\doxysection{Class\+: glob.\+Glob}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1319}
...
It\textquotesingle{}s an Event\+Emitter, and starts walking the filesystem to find matches immediately.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1320}{}\doxysubsection{new glob.\+Glob(pattern, \mbox{[}options\mbox{]}, \mbox{[}cb\mbox{]})}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1320}
...
Note that if the {\ttfamily sync} flag is set in the options, then matches will be immediately available on the {\ttfamily g.\+found} member.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1321}{}\doxysubsection{Properties}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1321}
...
\item `\textquotesingle{}FILE'{\ttfamily -\/ Path exists, and is not a directory $\ast$}\textquotesingle{}DIR\textquotesingle{}{\ttfamily -\/ Path exists, and is a directory $\ast$}\mbox{[}file, entries, ...\mbox{]}{\ttfamily -\/ Path exists, is a directory, and the array value is the results of}fs.\+readdir{\ttfamily  $\ast$}stat\+Cache{\ttfamily Cache of}fs.\+stat\`{} results, to prevent statting the same path multiple times.
\end{DoxyItemize}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1322}{}\doxysubsection{Events}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1322}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1323}{}\doxysubsection{Methods}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1323}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1324}{}\doxysubsection{Options}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1324}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1325}{}\doxysection{Comparisons to other fnmatch/glob implementations}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1325}
...
To specify things that should not match, use the {\ttfamily ignore} option.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1327}{}\doxysection{Windows}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1327}
...
Results from absolute patterns such as {\ttfamily /foo/$\ast$} are mounted onto the root setting using {\ttfamily path.\+join}. On windows, this will by default result in {\ttfamily /foo/$\ast$} matching {\ttfamily C\+:\textbackslash{}foo\textbackslash{}bar.\+txt}.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1328}{}\doxysection{Race Conditions}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1328}
...
Users are thus advised not to use a glob result as a guarantee of filesystem state in the face of rapid changes. For the vast majority of operations, this is never a problem.\hypertarget{md_src_nodejs_node_modules_glob_README_autotoc_md1329}{}\doxysection{Glob Logo}\label{md_src_nodejs_node_modules_glob_README_autotoc_md1329}
