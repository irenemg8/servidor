...
As of version 7, this is one of the most performant LRU implementations available in Java\+Script, and supports a wide diversity of use cases. However, note that using some of the features will necessarily impact performance, by causing the cache to have to do more work. See the \char`\"{}\+Performance\char`\"{} section below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1671}{}\doxysection{Installation}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1671}

\begin{DoxyCode}{0}
\DoxyCodeLine{npm install lru-\/cache -\/-\/save}

\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1672}{}\doxysection{Usage}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1672}
...
If you put more stuff in it, then items will fall out.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1673}{}\doxysection{Options}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1673}
\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1674}{}\doxysubsection{$<$tt$>$max$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1674}
...
{\bfseries{It is strongly recommended to set a {\ttfamily max} to prevent unbounded growth of the cache.}} See \char`\"{}\+Storage Bounds Safety\char`\"{} below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1675}{}\doxysubsection{$<$tt$>$max\+Size$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1675}
...
Even if size tracking is enabled, {\bfseries{it is strongly recommended to set a {\ttfamily max} to prevent unbounded growth of the cache.}} See \char`\"{}\+Storage Bounds Safety\char`\"{} below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1676}{}\doxysubsection{$<$tt$>$max\+Entry\+Size$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1676}
...
Optional, must be a positive integer if provided. Defaults to the value of {\ttfamily max\+Size} if provided.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1677}{}\doxysubsection{$<$tt$>$size\+Calculation$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1677}
...
Deprecated alias\+: {\ttfamily length}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1678}{}\doxysubsection{$<$tt$>$fetch\+Method$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1678}
...
For example, a DNS cache may update the TTL based on the value returned from a remote DNS server by changing {\ttfamily options.\+ttl} in the {\ttfamily fetch\+Method}.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1679}{}\doxysubsection{$<$tt$>$fetch\+Context$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1679}
Arbitrary data that can be passed to the {\ttfamily fetch\+Method} as the {\ttfamily context} option.

Note that this will only be relevant when the {\ttfamily cache.\+fetch()} call needs to call {\ttfamily fetch\+Method()}. Thus, any data which will meaningfully vary the fetch response needs to be present in the key. This is primarily intended for including {\ttfamily x-\/request-\/id} headers and the like for debugging purposes, which do not affect the {\ttfamily fetch\+Method()} response.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1680}{}\doxysubsection{$<$tt$>$no\+Delete\+On\+Fetch\+Rejection$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1680}
...
This may be set in calls to {\ttfamily fetch()}, or defaulted on the constructor, or overridden by modifying the options object in the {\ttfamily fetch\+Method}.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1681}{}\doxysubsection{$<$tt$>$allow\+Stale\+On\+Fetch\+Rejection$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1681}
...
This may be set in calls to {\ttfamily fetch()}, or defaulted on the constructor, or overridden by modifying the options object in the {\ttfamily fetch\+Method}.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1682}{}\doxysubsection{$<$tt$>$allow\+Stale\+On\+Fetch\+Abort$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1682}
...
Unless {\ttfamily ignore\+Fetch\+Abort} is also set, the underlying {\ttfamily fetch\+Method} will still be considered canceled, and its return value will be ignored and not cached.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1683}{}\doxysubsection{$<$tt$>$ignore\+Fetch\+Abort$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1683}
...
When used with {\ttfamily allow\+Stale\+On\+Fetch\+Abort}, aborted {\ttfamily fetch()} calls will resolve immediately to their stale cached value or {\ttfamily undefined}, and will continue to process and eventually update the cache when they resolve, as long as the resulting value is not {\ttfamily undefined}, thus supporting a \char`\"{}return stale on timeout while refreshing\char`\"{} mechanism by passing {\ttfamily Abort\+Signal.\+timeout(n)} as the signal.
...
\DoxyCodeLine{    const res = await fetch(`https://slow-\/src-\/server/\$\{key\}`)}
...
This may be overridden on the {\ttfamily fetch()} call or in the {\ttfamily fetch\+Method} itself.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1684}{}\doxysubsection{$<$tt$>$dispose$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1684}
...
Optional, must be a function.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1685}{}\doxysubsection{$<$tt$>$dispose\+After$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1685}
...
The {\ttfamily dispose\+After()} method is {\itshape not} called for canceled calls to {\ttfamily fetch\+Method()}. If you wish to handle evictions, overwrites, and deletes of in-\/flight asynchronous fetches, you must use the {\ttfamily Abort\+Signal} provided.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1686}{}\doxysubsection{$<$tt$>$no\+Dispose\+On\+Set$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1686}
...
Boolean, default {\ttfamily false}. Only relevant if {\ttfamily dispose} or {\ttfamily dispose\+After} options are set.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1687}{}\doxysubsection{$<$tt$>$ttl$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1687}
...
Even if ttl tracking is enabled, {\bfseries{it is strongly recommended to set a {\ttfamily max} to prevent unbounded growth of the cache.}} See \char`\"{}\+Storage Bounds Safety\char`\"{} below.
...
Deprecated alias\+: {\ttfamily max\+Age}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1688}{}\doxysubsection{$<$tt$>$no\+Update\+TTL$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1688}
...
Boolean, default false.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1689}{}\doxysubsection{$<$tt$>$ttl\+Resolution$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1689}
...
Note that setting this to a higher value {\itshape will} improve performance somewhat while using ttl tracking, albeit at the expense of keeping stale items around a bit longer than intended.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1690}{}\doxysubsection{$<$tt$>$ttl\+Autopurge$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1690}
...
Boolean, default {\ttfamily false}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1691}{}\doxysubsection{$<$tt$>$allow\+Stale$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1691}
...
Deprecated alias\+: {\ttfamily stale}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1692}{}\doxysubsection{$<$tt$>$no\+Delete\+On\+Stale\+Get$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1692}
...
Boolean, default false, only relevant if {\ttfamily ttl} is set.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1693}{}\doxysubsection{$<$tt$>$update\+Age\+On\+Get$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1693}
...
Boolean, default false, only relevant if {\ttfamily ttl} is set.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1694}{}\doxysubsection{$<$tt$>$update\+Age\+On\+Has$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1694}
...
Boolean, default false, only relevant if {\ttfamily ttl} is set.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1695}{}\doxysection{API}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1695}
\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1696}{}\doxysubsection{$<$tt$>$new LRUCache(options)$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1696}
Create a new LRUCache. All options are documented above, and are on the cache as public members.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1697}{}\doxysubsection{$<$tt$>$cache.\+max$<$/tt$>$, $<$tt$>$cache.\+max\+Size$<$/tt$>$, $<$tt$>$cache.\+allow\+Stale$<$/tt$>$,}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1697}
...
These are intended for read access only. Changing them during program operation can cause undefined behavior.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1698}{}\doxysubsection{$<$tt$>$cache.\+size$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1698}
The total number of items held in the cache at the current moment.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1699}{}\doxysubsection{$<$tt$>$cache.\+calculated\+Size$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1699}
The total size of items in cache when using size tracking.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1700}{}\doxysubsection{$<$tt$>$set(key, value, \mbox{[}\{ size, size\+Calculation, ttl, no\+Dispose\+On\+Set, start, status \}\mbox{]})$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1700}
...
For the usage of the {\ttfamily status} option, see {\bfseries{Status Tracking}} below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1701}{}\doxysubsection{$<$tt$>$get(key, \{ update\+Age\+On\+Get, allow\+Stale, status \} = \{\}) =$>$ value$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1701}
...
For the usage of the {\ttfamily status} option, see {\bfseries{Status Tracking}} below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1702}{}\doxysubsection{$<$tt$>$async fetch(key, options = \{\}) =$>$ Promise$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1702}
...
If a {\ttfamily signal} is passed to the {\ttfamily fetch()} call, then aborting the signal will abort the fetch and cause the {\ttfamily fetch()} promise to reject with the reason provided.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1703}{}\doxysubsection{$<$tt$>$peek(key, \{ allow\+Stale \} = \{\}) =$>$ value$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1703}
Like {\ttfamily get()} but doesn\textquotesingle{}t update recency or delete stale items.

Returns {\ttfamily undefined} if the item is stale, unless {\ttfamily allow\+Stale} is set either on the cache or in the options object.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1704}{}\doxysubsection{$<$tt$>$has(key, \{ update\+Age\+On\+Has, status \} = \{\}) =$>$ Boolean$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1704}
...
For the usage of the {\ttfamily status} option, see {\bfseries{Status Tracking}} below.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1705}{}\doxysubsection{$<$tt$>$delete(key)$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1705}
Deletes a key out of the cache.

Returns {\ttfamily true} if the key was deleted, {\ttfamily false} otherwise.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1706}{}\doxysubsection{$<$tt$>$clear()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1706}
Clear the cache entirely, throwing away all values.

Deprecated alias\+: {\ttfamily reset()}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1707}{}\doxysubsection{$<$tt$>$keys()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1707}
Return a generator yielding the keys in the cache, in order from most recently used to least recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1708}{}\doxysubsection{$<$tt$>$rkeys()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1708}
Return a generator yielding the keys in the cache, in order from least recently used to most recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1709}{}\doxysubsection{$<$tt$>$values()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1709}
Return a generator yielding the values in the cache, in order from most recently used to least recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1710}{}\doxysubsection{$<$tt$>$rvalues()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1710}
Return a generator yielding the values in the cache, in order from least recently used to most recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1711}{}\doxysubsection{$<$tt$>$entries()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1711}
Return a generator yielding {\ttfamily \mbox{[}key, value\mbox{]}} pairs, in order from most recently used to least recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1712}{}\doxysubsection{$<$tt$>$rentries()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1712}
Return a generator yielding {\ttfamily \mbox{[}key, value\mbox{]}} pairs, in order from least recently used to most recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1713}{}\doxysubsection{$<$tt$>$find(fn, \mbox{[}get\+Options\mbox{]})$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1713}
...
The optional {\ttfamily get\+Options} are applied to the resulting {\ttfamily get()} of the item found.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1714}{}\doxysubsection{$<$tt$>$dump()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1714}
...
Note\+: this returns an actual array, not a generator, so it can be more easily passed around.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1715}{}\doxysubsection{$<$tt$>$load(entries)$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1715}
...
The {\ttfamily start} fields are assumed to be calculated relative to a portable {\ttfamily Date.\+now()} timestamp, even if {\ttfamily performance.\+now()} is available.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1716}{}\doxysubsection{$<$tt$>$purge\+Stale()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1716}
Delete any stale entries. Returns {\ttfamily true} if anything was removed, {\ttfamily false} otherwise.

Deprecated alias\+: {\ttfamily prune}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1717}{}\doxysubsection{$<$tt$>$get\+Remaining\+TTL(key)$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1717}
Return the number of ms left in the item\textquotesingle{}s TTL. If item is not in cache, returns {\ttfamily 0}. Returns {\ttfamily Infinity} if item is in cache without a defined TTL.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1718}{}\doxysubsection{$<$tt$>$for\+Each(fn, \mbox{[}thisp\mbox{]})$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1718}
...
If {\ttfamily thisp} is provided, function will be called in the {\ttfamily this}-\/context of the provided object.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1719}{}\doxysubsection{$<$tt$>$rfor\+Each(fn, \mbox{[}thisp\mbox{]})$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1719}
Same as {\ttfamily cache.\+for\+Each(fn, thisp)}, but in order from least recently used to most recently used.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1720}{}\doxysubsection{$<$tt$>$pop()$<$/tt$>$}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1720}
Evict the least recently used item, returning its value.

Returns {\ttfamily undefined} if cache is empty.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1721}{}\doxysubsection{Internal Methods and Properties}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1721}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1722}{}\doxysection{Status Tracking}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1722}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1723}{}\doxysection{Storage Bounds Safety}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1723}
...
If that isn\textquotesingle{}t to your liking, check out \href{http://npm.im/@isaacs/ttlcache}{\texttt{ @isaacs/ttlcache}}.\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1724}{}\doxysection{Performance}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1724}
...
\end{DoxyEnumerate}\hypertarget{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1725}{}\doxysection{Breaking Changes in Version 7}\label{md_src_nodejs_node_modules_lru_cache_README_autotoc_md1725}
