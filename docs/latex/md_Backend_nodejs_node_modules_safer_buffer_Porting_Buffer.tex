\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_overview}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_overview}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2762}{}\doxysection{Overview}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2762}

\begin{DoxyItemize}
\item \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-1}{Variant 1\+: Drop support for Node.js ≤ 4.4.x and 5.0.0 — 5.9.x.}} ({\itshape recommended})
\item \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-2}{Variant 2\+: Use a polyfill}}
\item \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-3}{Variant 3\+: manual detection, with safeguards}}
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2763}{}\doxysubsection{Finding problematic bits of code using grep}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2763}
Just run `grep -\/nrE '\mbox{[}$^\wedge$a-\/z\+A-\/Z\mbox{]}(Slow)?Buffer\textbackslash{}s$\ast$(\textquotesingle{} --exclude-\/dir node\+\_\+modules\`{}.

It will find all the potentially unsafe places in your own code (with some considerably unlikely exceptions).\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2764}{}\doxysubsection{Finding problematic bits of code using Node.\+js 8}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2764}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2765}{}\doxysubsection{Finding problematic bits of code using linters}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2765}
...
\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-1}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-1}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2766}{}\doxysection{Variant 1\+: Drop support for Node.\+js ≤ 4.\+4.\+x and 5.\+0.\+0 — 5.\+9.\+x.}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2766}
...
{\itshape If you currently support those older Node.\+js versions and dropping them would be a semver-\/major change for you, or if you support older branches of your packages, consider using \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-2}{Variant 2}} or \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-3}{Variant 3}} on older branches, so people using those older branches will also receive the fix. That way, you will eradicate potential issues caused by unguarded Buffer API usage and your users will not observe a runtime deprecation warning when running your code on Node.\+js 10.}

\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-2}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-2}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2767}{}\doxysection{Variant 2\+: Use a polyfill}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2767}
Utilize \href{https://www.npmjs.com/package/safer-buffer}{\texttt{ safer-\/buffer}} as a polyfill to support older Node.\+js versions.

You would take exacly the same steps as in \mbox{\hyperlink{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-1}{Variant 1}}, but with a polyfill `const Buffer = require(\textquotesingle{}safer-\/buffer').Buffer{\ttfamily in all files where you use the new}Buffer\`{} api.
...
\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-3}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_variant-3}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2768}{}\doxysection{Variant 3 — manual detection, with safeguards}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2768}
This is useful if you create Buffer instances in only a few places (e.\+g. one), or you have your own wrapper around them.\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2769}{}\doxysubsection{Buffer(0)}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2769}
This special case for creating empty buffers can be safely replaced with {\ttfamily Buffer.\+concat(\mbox{[}$\,$\mbox{]})}, which returns the same result all the way down to Node.\+js 0.\+8.\+x.\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2770}{}\doxysubsection{Buffer(not\+Number)}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2770}
Before\+:
...
Also note that using Type\+Script does not fix this problem for you — when libs written in {\ttfamily Type\+Script} are used from JS, or when user input ends up there — it behaves exactly as pure JS, as all type checks are translation-\/time only and are not present in the actual JS code which TS compiles to.\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2771}{}\doxysubsection{Buffer(number)}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2771}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2772}{}\doxysection{Regarding Buffer.\+alloc\+Unsafe}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2772}
...
\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_faq}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_faq}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2773}{}\doxysection{FAQ}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2773}
\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_design-flaws}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_design-flaws}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2774}{}\doxysubsection{What is wrong with the $<$tt$>$\+Buffer$<$/tt$>$ constructor?}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2774}
...
\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_ecosystem-usage}%
\Hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_ecosystem-usage}%
 \hypertarget{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2775}{}\doxysubsection{The $<$tt$>$\+Buffer()$<$/tt$>$ constructor has been deprecated for a while. Is this really an issue?}\label{md_src_nodejs_node_modules_safer_buffer_Porting_Buffer_autotoc_md2775}
