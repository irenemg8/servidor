...
Looking to serve up entire folders mapped to URLs? Try \href{https://www.npmjs.org/package/serve-static}{\texttt{ serve-\/static}}.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2869}{}\doxysection{Installation}\label{md_src_nodejs_node_modules_send_README_autotoc_md2869}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2870}{}\doxysection{API}\label{md_src_nodejs_node_modules_send_README_autotoc_md2870}

\begin{DoxyCode}{0}
\DoxyCodeLine{var send = require('send')}

\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2871}{}\doxysubsection{send(req, path, \mbox{[}options\mbox{]})}\label{md_src_nodejs_node_modules_send_README_autotoc_md2871}
Create a new {\ttfamily Send\+Stream} for the given path to send to a {\ttfamily res}. The {\ttfamily req} is the Node.\+js HTTP request and the {\ttfamily path} is a urlencoded path to send (urlencoded, not the actual file-\/system path).\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2872}{}\doxysubsubsection{Options}\label{md_src_nodejs_node_modules_send_README_autotoc_md2872}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2873}{}\doxyparagraph{accept\+Ranges}\label{md_src_nodejs_node_modules_send_README_autotoc_md2873}
Enable or disable accepting ranged requests, defaults to true. Disabling this will not send {\ttfamily Accept-\/\+Ranges} and ignore the contents of the {\ttfamily Range} request header.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2874}{}\doxyparagraph{cache\+Control}\label{md_src_nodejs_node_modules_send_README_autotoc_md2874}
Enable or disable setting {\ttfamily Cache-\/\+Control} response header, defaults to true. Disabling this will ignore the {\ttfamily immutable} and {\ttfamily max\+Age} options.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2875}{}\doxyparagraph{dotfiles}\label{md_src_nodejs_node_modules_send_README_autotoc_md2875}
...
The default value is {\itshape similar} to `\textquotesingle{}ignore'\`{}, with the exception that this default will not ignore the files within a directory that begins with a dot, for backward-\/compatibility.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2876}{}\doxyparagraph{end}\label{md_src_nodejs_node_modules_send_README_autotoc_md2876}
Byte offset at which the stream ends, defaults to the length of the file minus 1. The end is inclusive in the stream, meaning {\ttfamily end\+: 3} will include the 4th byte in the stream.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2877}{}\doxyparagraph{etag}\label{md_src_nodejs_node_modules_send_README_autotoc_md2877}
Enable or disable etag generation, defaults to true.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2878}{}\doxyparagraph{extensions}\label{md_src_nodejs_node_modules_send_README_autotoc_md2878}
If a given file doesn\textquotesingle{}t exist, try appending one of the given extensions, in the given order. By default, this is disabled (set to {\ttfamily false}). An example value that will serve extension-\/less HTML files\+: `\mbox{[}\textquotesingle{}html', \textquotesingle{}htm\textquotesingle{}\mbox{]}\`{}. This is skipped if the requested file already has an extension.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2879}{}\doxyparagraph{immutable}\label{md_src_nodejs_node_modules_send_README_autotoc_md2879}
Enable or disable the {\ttfamily immutable} directive in the {\ttfamily Cache-\/\+Control} response header, defaults to {\ttfamily false}. If set to {\ttfamily true}, the {\ttfamily max\+Age} option should also be specified to enable caching. The {\ttfamily immutable} directive will prevent supported clients from making conditional requests during the life of the {\ttfamily max\+Age} option to check if the file has changed.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2880}{}\doxyparagraph{index}\label{md_src_nodejs_node_modules_send_README_autotoc_md2880}
By default send supports \char`\"{}index.\+html\char`\"{} files, to disable this set {\ttfamily false} or to supply a new index pass a string or an array in preferred order.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2881}{}\doxyparagraph{last\+Modified}\label{md_src_nodejs_node_modules_send_README_autotoc_md2881}
Enable or disable {\ttfamily Last-\/\+Modified} header, defaults to true. Uses the file system\textquotesingle{}s last modified value.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2882}{}\doxyparagraph{max\+Age}\label{md_src_nodejs_node_modules_send_README_autotoc_md2882}
Provide a max-\/age in milliseconds for http caching, defaults to 0. This can also be a string accepted by the \href{https://www.npmjs.org/package/ms\#readme}{\texttt{ ms}} module.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2883}{}\doxyparagraph{root}\label{md_src_nodejs_node_modules_send_README_autotoc_md2883}
Serve files relative to {\ttfamily path}.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2884}{}\doxyparagraph{start}\label{md_src_nodejs_node_modules_send_README_autotoc_md2884}
Byte offset at which the stream starts, defaults to 0. The start is inclusive, meaning {\ttfamily start\+: 2} will include the 3rd byte in the stream.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2885}{}\doxysubsubsection{Events}\label{md_src_nodejs_node_modules_send_README_autotoc_md2885}
...
\end{DoxyItemize}\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2886}{}\doxysubsubsection{.\+pipe}\label{md_src_nodejs_node_modules_send_README_autotoc_md2886}
The {\ttfamily pipe} method is used to pipe the response into the Node.\+js HTTP response object, typically {\ttfamily send(req, path, options).pipe(res)}.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2887}{}\doxysubsection{.\+mime}\label{md_src_nodejs_node_modules_send_README_autotoc_md2887}
...
This is used to configure the MIME types that are associated with file extensions as well as other options for how to resolve the MIME type of a file (like the default type to use for an unknown file extension).\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2888}{}\doxysection{Error-\/handling}\label{md_src_nodejs_node_modules_send_README_autotoc_md2888}
By default when no {\ttfamily error} listeners are present an automatic response will be made, otherwise you have full control over the response, aka you may show a 5xx page etc.\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2889}{}\doxysection{Caching}\label{md_src_nodejs_node_modules_send_README_autotoc_md2889}
It does {\itshape not} perform internal caching, you should use a reverse proxy cache such as Varnish for this, or those fancy things called CDNs. If your application is small enough that it would benefit from single-\/node memory caching, it\textquotesingle{}s small enough that it does not need caching at all ;).\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2890}{}\doxysection{Debugging}\label{md_src_nodejs_node_modules_send_README_autotoc_md2890}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2891}{}\doxysection{Running tests}\label{md_src_nodejs_node_modules_send_README_autotoc_md2891}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2892}{}\doxysection{Examples}\label{md_src_nodejs_node_modules_send_README_autotoc_md2892}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2893}{}\doxysubsection{Serve a specific file}\label{md_src_nodejs_node_modules_send_README_autotoc_md2893}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2894}{}\doxysubsection{Serve all files from a directory}\label{md_src_nodejs_node_modules_send_README_autotoc_md2894}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2895}{}\doxysubsection{Custom file types}\label{md_src_nodejs_node_modules_send_README_autotoc_md2895}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2896}{}\doxysubsection{Custom directory index view}\label{md_src_nodejs_node_modules_send_README_autotoc_md2896}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2897}{}\doxysubsection{Serving from a root directory with custom error-\/handling}\label{md_src_nodejs_node_modules_send_README_autotoc_md2897}
...
\end{DoxyCode}
\hypertarget{md_src_nodejs_node_modules_send_README_autotoc_md2898}{}\doxysection{License}\label{md_src_nodejs_node_modules_send_README_autotoc_md2898}
